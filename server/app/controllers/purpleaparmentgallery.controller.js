...
const PurpleApartmentGallery = db.purpleaparmentgallery;
...
exports.create = (req, res) => {
  ...
  const note = new PurpleApartmentGallery({
      title: req.body.title,
      picture:picture,
  });

  // Save PurpleApartmentGallery in the database
  note.save()
  .then(data => {
      res.send(data);
  }).catch(err => {
...
// Retrieve and return all PurpleApartmentGallery from the database.
exports.findAll = (req, res) => {
  PurpleApartmentGallery.find()
  .then(notes => {
    return res.json({
      purpleapartments:notes,
    });
  }).catch(err => {
      res.status(500).send({
...
exports.findOne = (req, res) => {
  PurpleApartmentGallery.findById(req.params.id)
  .then(note => {
      if(!note) {
...
exports.update = (req, res) => {
  ...
  PurpleApartmentGallery.findByIdAndUpdate(req.params.id, arr, { new: true })
  .then((page) => {
    if (!page) {
      return res.status(404).send({
...
exports.delete = (req, res) => {
  ...
  PurpleApartmentGallery.findByIdAndRemove(req.params.id)
  .then(note => {
     // if(!note) {
          //return res.status(404).send({
