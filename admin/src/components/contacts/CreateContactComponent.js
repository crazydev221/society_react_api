...
this.saveOrUpdateContact = this.saveOrUpdateContact.bind(this);
  ...
  componentDidMount() {
    ...
    if(this.state._id === '_add'){
      ...
      ContactService.getById(this.state._id).then( (res) =>{
          let Contact = res.data;
          this.setState({first_name: Contact.first_name,last_name: Contact.last_name,email:Contact.email, phone: Contact.phone,  message: Contact.message
          });
      });
  }  
  }
  ...
  saveOrUpdateContact = (e) => {
    ...
    if(this.state._id === '_add'){
        ...
        ContactService.update(data, this.state._id).then( res => {
            this.props.history.push('/contacts');
        });
    }
}
...
renderPreview() {
  if (this.state.src) {
    ...
    return <img class="main-img" src={HttpCommon.defaults.imageURL+'/uploads/Contacts/'+this.state.picture} />;
  } else {
    return "";
  }
}
