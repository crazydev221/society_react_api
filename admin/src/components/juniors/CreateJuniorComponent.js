...
export default class CreateJuniorComponent extends Component {
    ...
    saveOrUpdateOurWork = (e) => {
        ...
        if (this.state._id === '_add') {
            OurWorkService.create(formData).then(res => {
                this.props.history.push('/juniors');
            });
        } else {
    ...
    getTitle() {
        if (this.state._id === '_add') {
            return <h2 className="text-center">Add Junior Purple Society</h2>
        } else {
            return <h2 className="text-center">Update Junior Purple Society</h2>
        }
    }
    ...
    renderPreview() {
        if (this.state.src) {
            ...
            return <img class="main-img"
                        src={HttpCommon.defaults.imageURL + '/uploads/juniorpurplesocieties/' + this.state.picture}/>;
        } else {
            return "";
        }
    }
