...
export default class CreatePurpleApartmentComponent extends Component {
    constructor(props) {
        super(props);
    ...
    saveOrUpdateOurWork = (e) => {
        ...
        if (this.state._id === '_add') {
            OurWorkService.create(formData).then(res => {
                this.props.history.push('/purpleapartments');
            });
        } else {
    ...
    getTitle() {
        if (this.state._id === '_add') {
            return <h2 className="text-center">Add Purple Apartment</h2>
        } else {
            return <h2 className="text-center">Update Purple Apartment</h2>
        }
    }
    ...
    renderPreview() {
        if (this.state.src) {
            ...
            return <img class="main-img"
                        src={HttpCommon.defaults.imageURL + '/uploads/purpleapartments/' + this.state.picture}/>;
        } else {
            return "";
        }
    }
